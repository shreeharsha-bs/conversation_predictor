...
ngram-count -lm $tgtdir/5gram.kn0223.gz -kndiscount1 -gt1min 0 -kndiscount2 -gt2min 2 -kndiscount3 -gt3min 2 -kndiscount4 -gt4min 3 -kndiscount5 -gt4min 3 -order 5 -text $tgtdir/train.txt -vocab $tgtdir/vocab -unk -sort
...
#We look at the two lowest perplexity LMs and use a 4gram LM if one of the two, even if the 4gram is of lower ppl# nah use 5 gram
nof_trigram_lm=`head -n 2 $tgtdir/perplexities.txt | grep 5gram | wc -l`
if [[ $nof_trigram_lm -eq 0 ]] ; then
  ...
  lmfilename=`head -n 2 $tgtdir/perplexities.txt | grep 5gram | cut -f 1 -d ' '` 
fi
